...
### 3. 安装 Git Hooks（可选，推荐）

```bash
# 安装自动更新README的git hook
npm run install-hooks
```

> 💡 **提示**：安装后，每次提交代码时会自动更新README的时间戳。如果跳过此步骤，可以手动运行 `npm run update-readme` 来更新README。
...
- 📝 **资源管理** - 添加、编辑、删除资源卡片
  ...
  - **批量新增**：支持 Excel 批量导入资源（在新增卡片中点击"批量新增"）
    ...
    - 自动解析和验证数据
    - 支持批量保存多条资源
  - **Learning 分类特殊功能**：编辑资源时可以移动到其他分类（AiCC、UXTips、星芒学社、图库）
...
- 🔑 **密钥管理** - 生成和管理 星芒学社 访问密钥
  - 设置密钥有效期
  - 设置密钥类型（普通用户/管理员）
  - **星芒学社可见度控制**：设置用户可查看的资源百分比（20%、50%、100%）
    - 20%：用户只能看到前 20% 的资源
    - 50%：用户只能看到前 50% 的资源
    - 100%：用户可以看到所有资源
...
- ⚙️ **头部配置** - 自定义 Logo、标题、联系图片
  ...
  - 自定义标题文字
  - 配置合作交流图片
  - 配置存储在浏览器本地存储（localStorage）中，无需后端支持
  - 上传头像图片
  - 自定义标题文字
  - 配置合作交流图片
...
| 用户类型 | AiCC | UXTips | Learning | 星芒学社 | 图库 |
|---------|------|-------|----------|-----------|------|
| 游客 | ✅ | ✅ | ❌ | ✅ (20%) | ✅ (20%) |
| 星芒学社密钥 | ✅ | ✅ | ❌ | ✅ (按密钥设置) | ✅ (100%) |
| 管理员 | ✅ | ✅ | ✅ | ✅ (100%) | ✅ (100%) |
...
> 💡 **星芒学社 访问说明**：
> - **未登录用户**：可以访问星芒学社，但只能看到 20% 的内容，底部会提示"如需学习更多，请联系管理员卡卡"
> - **已登录用户**：根据密钥的可见度设置显示内容（20%、50% 或 100%）
...
- ✅ **权限控制** - 管理员和访问密钥双重权限
- ✅ **可见度控制** - 支持为密钥设置星芒学社资源可见度百分比（20%、50%、100%）
...
5. 按照模板格式填写资源信息：
   ...
   - **分类值**：必须是 AiCC、UXTips、Learning、星芒学社、图库 之一
6. 上传填写好的 Excel 文件
7. 系统自动解析并显示预览
8. 检查错误提示（如有）
9. 点击"批量保存"完成导入
