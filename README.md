...
> 💡 **权限说明**：
> - **AiCC 和 UXTips**：所有用户（包括游客）都可以查看全部内容，无需登录
> - **星芒学社和图库**：
...
- `POST /api/config/header` - 保存头部配置（需管理员认证）
  - 请求体：`{ avatar: string, avatarImage?: string, title: string, contactImage?: string, cooperationImage?: string }`
  - 保存后所有用户都会看到最新配置
  - 配置存储在数据库中
...
- ✅ **分类图标** - 每个分类都有专属彩色 emoji 图标（AiCC: 🤖、UXTips: 🎨、Learning: 📚、星芒学社: ✨、图库: 🖼️）
