...
export function BatchAddForm({ filters, onSave, onCancel, showHeader = true }: BatchAddFormProps) {
  ...
  const validCategories: CategoryType[] = ['AiCC', 'UXLib', 'Learning', 'Starlight Academy'];
  ...
  const parseExcelFile = async (file: File) => {
    ...
    try {
      ...
      jsonData.forEach((row, index) => {
        ...
        // 解析内容类型
        const contentType = row.内容类型?.toString().toLowerCase().trim() === 'document' ? 'document' : 'link';
        ...
        const resource = {
          ...
          content: row.文档内容?.toString().trim() || '',
        };
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              ...
              <ul className="list-disc list-inside space-y-1 text-xs text-blue-800">
                ...
                <li><strong>可选列：</strong>描述、标签（用逗号分隔）、图片链接、跳转链接、卡卡推荐（true/是/1）、内容类型（link/document）、文档内容</li>
                <li><strong>分类值：</strong>必须是 AiCC、UXLib、Learning、Starlight Academy 之一</li>
