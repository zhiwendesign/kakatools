...
export function Header({
  ...
  const allCategories: { id: CategoryType; label: string; special?: boolean; adminOnly?: boolean }[] = [
    { id: 'AiCC', label: 'AiCC' },
    { id: 'UXTips', label: 'UXTips' },
    { id: 'Learning', label: 'Learning', adminOnly: true },
    { id: '星芒学社', label: '星芒学社', special: true },
    { id: '图库', label: '图库', special: true },
  ];
  ...
  // - Guest: can see AiCC, UXTips, 星芒学社 (but only 20% of 星芒学社 content), and 图库
  const categories = allCategories.filter(cat => {
    if (cat.adminOnly) {
      return hasAdminAccess; // Learning requires admin access (admin login or admin-type key)
    }
    return true; // AiCC, UXTips, 星芒学社, and 图库 are visible to everyone
  });
