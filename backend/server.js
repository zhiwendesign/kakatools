...
app.post('/api/resources/batch', requireAuth, (req, res, next) => {
  try {
    ...
    if (results.success > 0) {
      // 清除相关缓存
      const successfulCategories = new Set(resourcesList.filter((_, i) => results.errors.every(e => e.index !== i + 1)).map(r => r.category));
      successfulCategories.forEach(cat => dataCache.delete(`resources:${cat}`));
      dataCache.delete('resources:all');
...
app.post('/api/filters', requireAuth, (req, res, next) => {
  try {
    ...
    if (success) {
      // 清除相关缓存
      dataCache.delete(`resources:${category}`);
      dataCache.delete('resources:all');
...
const dataCache = new AdvancedCache({
  ...
  cleanupInterval: 60 * 1000 // 每1分钟清理一次
});
